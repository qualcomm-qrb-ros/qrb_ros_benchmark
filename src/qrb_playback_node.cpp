...
void QrbPlaybackNode::create_message_pubsub(const size_t index, const std::string format)
{
  ...
  auto subscriber =
      this->create_subscription<ROSMessageType>("buffer/input" + std::to_string(index),
          ros2_benchmark::kBufferQoS, subscriber_callback, subscription_options);
...
template <typename ROSMessageType>
void QrbPlaybackNode::message_subscriber_callback(const std::shared_ptr<ROSMessageType> msg,
    const size_t index)
{
  ...
  RCLCPP_DEBUG(this->get_logger(),
      "Add the message to the buffer, current capacity (%ld/%ld), index=%ld", size,
      requested_buffer_length_, index);
...
bool QrbPlaybackNode::PublishMessage(const size_t pub_index,
    ...
    const std::optional<std_msgs::msg::Header> & header = std::nullopt)
{
  ...
  if (message_index >= size) {
    RCLCPP_ERROR(get_logger(),
        "Failed to publish message, message index: %ld, "
...
template <typename ROSMessageType>
bool QrbPlaybackNode::publish_message(const size_t pub_index,
    const size_t message_index,
